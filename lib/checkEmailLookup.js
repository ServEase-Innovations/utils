...
/** Customer rows created on first Auth0 login have no mobile — not a true dual-role user. */
function hasRegisteredMobile(row) {
  if (row?.mobileno == null) return false;
  const digits = String(row.mobileno).replace(/\D/g, "");
  return digits.length >= 10;
}

async function lookupUserByEmail(pool, rawEmail) {
  ...
  if (customer && sp) {
    if (!hasRegisteredMobile(customer)) {
      return {
...
module.exports = { normalizeLoginEmail, lookupUserByEmail };
